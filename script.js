...
let currentUnit = 'celsius'; 
let currentWeatherData = null; 
let tempChartInstance = null; // ⭐ Added for Temperature Chart
let aqiChartInstance = null; 
const cityCoordinatesCache = {}; 
...
const clearWeatherUI = (isInitial = false) => {
    ...
    // if (mapInstance) { mapInstance.remove(); mapInstance = null; } // Map not implemented in this version
    if (aqiChartInstance) { aqiChartInstance.destroy(); aqiChartInstance = null; }
    if (tempChartInstance) { tempChartInstance.destroy(); tempChartInstance = null; } // ⭐ Added
};
...
const updateWeatherUI = (data, lat, lon) => {
    ...
    // renderMap(lat, lon, data.city); // Map not implemented in this version
    renderAqiChart(data.details.aqiIndex);
    renderTempChart(data.forecasts.hourly); // ⭐ Added call for Temperature Chart
...
const renderTempChart = (hourlyForecast) => {
    if (typeof Chart === 'undefined') {
        return;
    }
    ...
    // Prepare data from hourly forecast
    const labels = hourlyForecast.map(item => item.time);
    const dataPoints = hourlyForecast.map(item => {
...
// ⭐ PARSER - Chatbot text को structured data में बदलने का लॉजिक
const parseWeatherReport = (text) => {
    const normalizedText = (text || '').trim();
    ...
    // 1. City, Temperature, and Description Extraction
    const cityMatch = normalizedText.match(/weather in\s*(.+?)\s*(?:is|hai|mein)/i);
    if (cityMatch && cityMatch[1]) {
        let city = cityMatch[1].trim().replace(/, India|,\s*is|\s*is|\.$/gi, '').trim();
        city = city.split(' is ')[0].split(' hai ')[0].trim();
    ...
    const descriptionMatch = normalizedText.match(/(?:°C|°F|\s*C|\s*F)\s*and\s*([^.]+)\. Details:/i) ||
                             normalizedText.match(/(?:°C|°F|\s*C|\s*F)\s*aur\s*([^.]+)\.\s*Details:/i);
    ...
    // --- Mock Forecast Data (Fallback if no real API is used) ---
    if (data.temp.current !== 'N/A' && !isNaN(parseFloat(data.temp.current))) {
        const baseTemp = parseFloat(data.temp.current);
        const desc = data.description !== 'N/A' ? data.description : 'clear sky';
        ...
        const hourlyTimes = [12, 15, 18, 21, 24]; 
        data.forecasts.hourly = hourlyTimes.map((h, index) => {
            const tempChange = index > 2 ? -2 : index % 2 + 1; 
            let timeLabel = h === 12 ? '12 PM' : h === 24 ? '12 AM' : `${h % 12} PM`;
            if (h < 12) timeLabel = `${h} AM`;
...
if (unitToggle) {
    unitToggle.addEventListener('click', () => {
        ...
        if (currentUnit === 'celsius') {
            currentUnit = 'fahrenheit';
            spanEl.textContent = 'Switch to °C';
            unitToggle.querySelector('i').setAttribute('data-lucide', 'thermometer-sun');
        } else {
            currentUnit = 'celsius';
            spanEl.textContent = 'Switch to °F';
            unitToggle.querySelector('i').setAttribute('data-lucide', 'thermometer');
        }
        // Re-render UI with saved data and new unit
        if (currentWeatherData) {
            const city = currentWeatherData.city;
...
if (hourlyTabButton && hourlyForecastSection) {
    hourlyTabButton.addEventListener('click', () => {
        ...
        // Add 'active-nav-item' to the clicked button
...
// ⭐ SCROLLING LOGIC FOR TODAY
document.querySelector('[data-section="TODAY"]').addEventListener('click', function() {
    document.querySelectorAll('.nav-item').forEach(btn => btn.classList.remove('active-nav-item'));
    this.classList.add('active-nav-item');
    smoothScrollTo(document.getElementById('todayDetailsSection'));
});
...
window.onload = () => {
    ...
    // Set a default city and run the initial search
    searchCityInput.value = 'New York, US';
    handleSearchSubmit();
};